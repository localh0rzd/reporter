...
def get_parent(projects, project, path=[]):
    if project and project["parent"]:
        path.append(project)
        return get_parent(projects, next((x for x in projects if x["id"] == path[-1]), None), path)
    else:
        print(len(path))
        return get_parent(projects, next((x for x in projects if x["id"] == path[-1]), None))
...
def get_projects():
    ...
    #body = list(filter(lambda x: x['active'], body))
    seen = []
    if not body:
        print("RIP")
        return
    def traverse(proj, acc = []):
        #acc += f"{proj['id']}: {proj['name']}"
        seen.append(proj["id"])
        if not proj['active']:
    ...
    for project in body[1:]:
        ...
        if not project['active']:
            ...
            continue
        seen.append(project['id'])
        print(f'traverse project {project["id"]}')
